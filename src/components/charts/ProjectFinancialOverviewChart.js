...
function ProjectOverviewTooltip({ active, payload }) {
...
export default function ProjectFinancialOverviewChart({ recebido = 0, custosDespesas = 0, tributos = 0, resultado = 0 }) {
  const data = [
    { name: 'Recebido Líquido', value: recebido },
    { name: 'Custos + Despesas', value: custosDespesas },
    ...
    <div style={{ width: '100%', height: '250px', minHeight: 0 }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 10, right: 18, left: 8, bottom: 24 }} barCategoryGap="28%">
          ...
          <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} width={125} tick={{ fill: 'var(--text-main)', fontSize: 10.5, fontWeight: 650 }} />
          <ReferenceLine x={0} stroke="var(--border-color)" strokeWidth={1} />
          <Tooltip content={<ProjectOverviewTooltip />} cursor={{ fill: 'rgba(255,255,255,0.025)' }} />
