...
export default function ProjectComparisonChart({ data, keys, colors, names }) {
  if (!data || data.length === 0) {
    return <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)', fontSize: '13px' }}>Sem dados para comparação.</div>;
  }
    ...
    return (
      <div style={{ backgroundColor: 'var(--bg-elevated)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '1rem', fontSize: '12px', color: 'var(--text-main)', boxShadow: '0 4px 6px rgba(0,0,0,0.3)' }}>
        ...
        {payload.map((entry, index) => (
          <p key={index} style={{ color: entry.color, display: 'flex', justifyContent: 'space-between', gap: '1rem' }}><span>{entry.name}:</span><strong>{formatCurrency(entry.value)}</strong></p>
        ))}
      </div>
    );
  };
  ...
  return (
    <div style={{ width: '100%', height: '100%', minHeight: '350px' }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ top: 5, right: 150, left: 30, bottom: 5 }} barCategoryGap="20%">
          ...
          {keys.map((key, index) => (
            <Bar key={key} dataKey={key} name={names[index] || key} fill={colors[index]} radius={[0, 4, 4, 0]}>
              <LabelList dataKey={key} position="right" formatter={(val) => val ? formatCurrency(val) : ''} style={{ fill: 'var(--text-secondary)', fontSize: '10px', fontWeight: '500' }} />
            </Bar>
          ))}
        </BarChart>
      </ResponsiveContainer>
