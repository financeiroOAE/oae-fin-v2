...
if (!content.includes(oldBlock)) {
  throw new Error('Referencia residual rawProjectRevenueStats nao encontrada');
}
...
if (!content.includes(oldDeps)) {
  throw new Error('Dependencia residual rawProjectRevenueStats nao encontrada');
}
content = content.replace(oldDeps, newDeps);

if (content.includes('rawProjectRevenueStats')) {
  throw new Error('Ainda existe referencia a rawProjectRevenueStats em Projetos');
}

fs.writeFileSync(path, content);
console.log('Referencia residual removida; Projetos usa os totais consolidados.');
