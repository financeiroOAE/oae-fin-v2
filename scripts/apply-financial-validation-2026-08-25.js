...
function replaceRegexOrFail(src, regex, replacement, label) {
  if (!regex.test(src)) throw new Error(`Trecho regex nao encontrado: ${label}`);
...
`  const projetosCruzados = useMemo(() => {
    ...
    projectCashData.forEach((item) => {
      ...
      // Realizados obedecem ao período selecionado. Em aberto é uma posição atual e
      // não pode desaparecer apenas porque o vencimento está depois da data final.
...
`  const dreStats = useMemo(() => {
    ...
    data.forEach((item) => {
      ...
      const dreInfo = `${'${item.dreClasse || \'\'}'} ${'${item.dreLinha || \'\'}'} ${'${item.dreDescricao || \'\'}'}`.toUpperCase();
      ...
      if (isRevenueTax(item)) {
        ...
        // Demais saídas com DEPARA válido pertencem à composição financeira do projeto.
...
`  const topEntradasData = useMemo(() =>
  ...
  src = src.replace(/Caixa: Data Inicial/g, 'Data Inicial');
  src = src.replace(/Caixa: Data Final/g, 'Data Final');
  src = src.replace(/Impostos sobre Faturamento/g, 'Tributos sobre Receita e Lucro');
  src = src.replace(/do faturamento total/g, 'da receita de projetos');
...
`  const openFilteredData = useMemo(() => baseData.filter((item) => {
  ...
  src = replaceRegexOrFail(
    src,
    /  const entryStatusBreakdown = useMemo\(\(\) => \{[\s\S]*?\n  \}, \[filteredData\]\);/,
`  const entryStatusBreakdown = useMemo(() => {
  ...
  src = replaceRegexOrFail(
    src,
    /  const projectFinancialOverview = useMemo\(\(\) => \{[\s\S]*?\n  \}, \[filteredData, activeProjectKeys\]\);/,
...
`            <h2 style={{ fontSize: '15px', fontWeight: '600', color: 'var(--text-main)', marginBottom: '0.25rem' }}>Visão Financeira Geral dos Projetos</h2>
  ...
  src = src.replace('Receitas de projetos versus custos e despesas realizados no período.', 'Recebimentos de projetos (obra + administrativo vinculado), saídas das obras, resultado e margem no período.');
  ...
  src = src.replace(/gridTemplateColumns: 'repeat\(auto-fit, minmax\(450px, 1fr\)\)'/g, "gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 420px), 1fr))'");
...
// -----------------------------------------------------------------------------
{
  ...
  src = src.replace('<table style={{ fontSize: \'12px\' }}>', '<table style={{ fontSize: \'12px\', minWidth: \'760px\' }}>');
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) walk(full);
      ...
      const updated = original
        .replace(/Caixa:\s*Data Inicial/g, 'Data Inicial')
        .replace(/Caixa:\s*Data Final/g, 'Data Final');
...
console.log('Correcoes financeiras e de UI aplicadas.');
